...
def build_effective_request_spec(api_request: ApiRequest, test_case: ApiTestCase | None = None) -> EffectiveRequestSpec:
    ...
    if test_case:
        override_spec = serialize_test_case_override(test_case)
        if serialize_extractor_specs(test_case):
            extractor_specs = extractor_specs + serialize_extractor_specs(test_case)
        request_spec = {
            **request_spec,
            "method": override_spec.get("method") or request_spec["method"],
            "url": override_spec.get("url") or request_spec["url"],
            "body_mode": override_spec.get("body_mode") or request_spec["body_mode"],
            "body_json": override_spec.get("body_json") or request_spec["body_json"],
            "raw_text": override_spec.get("raw_text") or request_spec["raw_text"],
            "xml_text": override_spec.get("xml_text") or request_spec["xml_text"],
            "binary_base64": override_spec.get("binary_base64") or request_spec["binary_base64"],
            "graphql_query": override_spec.get("graphql_query") or request_spec["graphql_query"],
            "graphql_operation_name": override_spec.get("graphql_operation_name") or request_spec["graphql_operation_name"],
            "graphql_variables": override_spec.get("graphql_variables") or request_spec["graphql_variables"],
            "timeout_ms": override_spec.get("timeout_ms") or request_spec["timeout_ms"],
            "headers": _merge_named_items(request_spec["headers"], override_spec["headers"]),
            "query": _merge_named_items(request_spec["query"], override_spec["query"]),
            "cookies": _merge_named_items(request_spec["cookies"], override_spec["cookies"]),
            "form_fields": _merge_named_items(request_spec["form_fields"], override_spec["form_fields"]),
            "multipart_parts": _merge_named_items(request_spec["multipart_parts"], override_spec["multipart_parts"]),
            "files": _merge_file_items(request_spec["files"], override_spec["files"]),
            "auth": _merge_auth(request_spec["auth"], override_spec["auth"]),
            "transport": _merge_transport(request_spec["transport"], override_spec["transport"]),
        }
...
def evaluate_structured_assertions(
    ...
    for index, spec in enumerate(assertion_specs, start=1):
        ...
        if assertion_type == "status_code":
            ...
            passed = actual == expected if operator == "equals" else str(expected) in str(actual or "")
            ...
            if operator == "gte":
                passed = actual is not None and expected_number is not None and actual >= expected_number
            elif operator == "lte":
                passed = actual is not None and expected_number is not None and actual <= expected_number
            else:
                passed = actual == expected_number
...
def execute_api_request(
    *,
    ...
    effective_spec = build_effective_request_spec(api_request, test_case)
    ...
    request_snapshot = {
        ...
        "body": resolved_body,
        "request_spec": serialize_request_spec(api_request),
        "request_override_spec": serialize_test_case_override(test_case) if test_case else None,
    ...
    if missing_request_variables:
        ...
        try:
            client = run_context.get_client(transport)
